...
class MemoryPool {
  public:
	MemoryPool()
	{
		...
		// lock_guard<mutex> lk(mtx); // avoid race condition
	...
	void* allocate(size_t size)
	{
		...
		// lock_guard<mutex> lk(array_mtx); // avoid race condition for continuous memory
	...
	{
		std::lock_guard<std::mutex> lk(mtx); // avoid race condition
		// lock_guard<mutex> lk(mtx); // avoid race conditio
		// free object memory
