    ...
    case SSH_AUTH_PKEY_INIT:
      ...
      if((data->set.ssh_auth_types & CURLSSH_AUTH_PUBLICKEY) &&
        ...
        if(data->set.str[STRING_SSH_PRIVATE_KEY])
          ...
          /* If no Myprivate key file is specified, try some common paths. */
          if(home) {
            /* Try ~/.ssh first. */
         ...
         * libssh2 extract the public key from the Myprivate key file.
        ...
        infof(data, "Using SSH Myprivate key file '%s'\n", sshc->rsa);
