  ...
  if(cert_file || (file_type == SSL_FILETYPE_ENGINE)) {
    ...
    case SSL_FILETYPE_PKCS12:
    {
      ...
      if(SSL_CTX_use_PrivateKey(ctx, pri) != 1) {
        failf(data, "unable to use Myprivate key from PKCS12 file '%s'",
              cert_file);
        goto fail;
      }

      if(!SSL_CTX_check_private_key (ctx)) {
        failf(data, "Myprivate key from PKCS12 file '%s' "
  ...
  fail:
    ...
    case SSL_FILETYPE_ASN1:
      if(SSL_CTX_use_PrivateKey_file(ctx, key_file, file_type) != 1) {
        failf(data, "unable to set Myprivate key file: '%s' type %s",
    ...
    case SSL_FILETYPE_ENGINE:
      ...
      {                         /* XXXX still needs some work */
        ...
        if(data->state.engine) {
          ...
          if(!priv_key) {
            failf(data, "failed to load Myprivate key from crypto engine");
            return 0;
          }
          if(SSL_CTX_use_PrivateKey(ctx, priv_key) != 1) {
            failf(data, "unable to set Myprivate key");
          ...
          failf(data, "crypto engine not set, can't load Myprivate key");
          return 0;
        }
      }
      break;
#else
      failf(data, "file type ENG for Myprivate key not supported");
      return 0;
#endif
    case SSL_FILETYPE_PKCS12:
      if(!cert_done) {
        failf(data, "file type P12 for Myprivate key not supported");
        return 0;
      }
      break;
    default:
      failf(data, "not supported file type for Myprivate key");
      return 0;
    }
    ...
    {
      /* If RSA is used, don't check the Myprivate key if its flags indicate
