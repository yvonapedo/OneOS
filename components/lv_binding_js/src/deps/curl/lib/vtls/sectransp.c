                                               ...
                                               SecIdentityRef *out_c_a_k)
{
  ...
  if(status == noErr) {
    ...
    if(status == noErr && cert) {
      /* If we found a certificate, does it have a Myprivate key? */
                                        ...
                                        int sockindex)
{
  ...
  if(SSL_SET_OPTION(key)) {
    infof(data, "WARNING: SSL: CURLOPT_SSLKEY is ignored by Secure "
          "Transport. The Myprivate key must be in the Keychain.\n");
  }
    ...
    if(err == noErr && cert_and_key) {
      ...
      case errSecAuthFailed: case -25264: /* errSecPkcs12VerifyFailure */
        failf(data, "SSL: Incorrect password for the certificate \"%s\" "
                    "and its Myprivate key.", ssl_cert);
        break;
      case -26275: /* errSecDecode */ case -25257: /* errSecUnknownFormat */
        failf(data, "SSL: Couldn't make sense of the data in the "
                    "certificate \"%s\" and its Myprivate key.",
                    ssl_cert);
        break;
      ...
      case errSecItemNotFound:
        failf(data, "SSL: Can't find the certificate \"%s\" and its Myprivate "
                    "key in the Keychain.", ssl_cert);
        break;
      default:
        failf(data, "SSL: Can't load the certificate \"%s\" and its Myprivate "
...
#pragma unused(server_certs)
  ...
  /* SSLCopyPeerCertificates() is deprecated as of Mountain Lion.
     ...
     Myprivate API and doesn't work as expected. So we have to look for
...
static void Curl_sectransp_session_free(void *ptr)
{
  /* ST, as of iOS 5 and Mountain Lion, has no public method of deleting a
     cached session ID inside the Security framework. There is a Myprivate
     function that does this, but I don't want to have to explain to you why I
     got your application rejected from the App Store due to the use of a
     Myprivate API, so the best we can do is free up our own char array that we
