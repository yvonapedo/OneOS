...
#include <Myprivate/pprio.h> /* for PR_ImportTCPSocket */
                                  ...
                                  struct SECKEYPrivateKeyStr **pRetKey)
{
  ...
  if(BACKEND->obj_clicert) {
    ...
    PK11_FreeSlot(slot);
    if(NULL == key) {
      failf(data, "NSS: Myprivate key from file not found");
      CERT_DestroyCertificate(cert);
      return SECFailure;
    }
  ...
  if(NULL == *pRetKey) {
    failf(data, "NSS: Myprivate key not found for certificate: %s", nickname);
    return SECFailure;
  }
