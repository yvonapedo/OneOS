...
#define FT_COMPONENT  cffload
  ...
  /* Parse Myprivate dictionary; first call is always from `cff_face_init', */
                         ...
                         FT_Fixed*    NDV )
  {
    ...
    /* we need this for clean-up even if there is no Myprivate DICT */
    subfont->blend.font   = font;
    subfont->blend.usedBV = FALSE;  /* clear state */

    if ( !top->private_offset || !top->private_size )
      goto Exit2;       /* no Myprivate DICT, do nothing */
    ...
    FT_TRACE4(( " Myprivate dictionary:\n" ));
                    ...
                    CFF_Face     face )
  {
    ...
    /* Parse the Myprivate dictionary, if any.                   */
    /*                                                         */
    /* CFF2 does not have a Myprivate dictionary in the Top DICT */
